...
// Decompose the file path. The syntaxe is 'Archive.ext#subfile', or 'Archive.ext', or '#subfile'
    ...
    function BeforeShow(&$Render, $File = '') {
        ...
        switch ($this->ExtEquiv) {
            case 'ods': $this->OpenDoc_SheetSlides_DeleteAndDisplay(true);
                break;
            case 'odp': $this->OpenDoc_SheetSlides_DeleteAndDisplay(false);
                break;
            case 'xlsx': $this->MsExcel_SheetDeleteAndDisplay();
                break;
            case 'pptx': $this->MsPowerpoint_SlideDelete();
                break;
            case 'docx': $this->MsWord_RenumDocPr();
                break;
        }
        ...
        if ($this->OpenXmlRid !== false)
            $this->OpenXML_RidCommit($Debug); // Must be done also after the loop because some Rid can be added with [onshow]
                ...
                if ($this->LastReadNotStored) {
                    ...
                    if ($this->LastReadComp <= 0) { // the contents is not compressed
                        if ($this->ExtInfo !== false) {
                            ...
                            $e = $this->ExtEquiv;
                            if (isset($i['rpl_what']))
                                $TBS->Source = str_replace($i['rpl_what'], $i['rpl_with'], $TBS->Source); // auto replace strings in the loaded file
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
    ...
    function TbsDebug_Info($Exit) {
        ...
        switch ($this->ExtEquiv) {
            case 'docx': $this->MsWord_DocDebug($nl, $sep, $bull);
                break;
            case 'xlsx': $this->MsExcel_SheetDebug($nl, $sep, $bull);
                break;
            case 'pptx': $this->MsPowerpoint_SlideDebug($nl, $sep, $bull);
                break;
            case 'ods' : $this->OpenDoc_SheetSlides_Debug(true, $nl, $sep, $bull);
                break;
            case 'odp' : $this->OpenDoc_SheetSlides_Debug(false, $nl, $sep, $bull);
                break;
        }

        switch ($this->ExtType) {
            case 'openxml': $this->OpenXML_ChartDebug($nl, $sep, $bull);
                break;
            case 'odf': $this->OpenDoc_ChartDebug($nl, $sep, $bull);
                break;
        }
    ...
    function TbsPicFound($Txt, &$Loc, $IsCaching) {
        ...
        if ($this->ExtType === 'odf') {
            ...
            if (isset($Loc->PrmLst['adjust']))
                $Loc->otbsDim = $this->TbsPicGetDim_ODF($Txt, $Loc->PosBeg, !$backward, $shift);
    ...
    function TbsPicGetDim_OpenXML_dml($Txt, $Pos, $Forward, $Shift) {
        ...
        if (($dim_inner !== false) && ($dim_inner['wb'] < $dim_shape['wb']))
            $dim_inner = false; // <a:ext> isoptional but must always be after the corresponding <wp:extent>, otherwise it may be the <a:ext> of another picture
    ...
    function TbsDeleteColumns(&$Txt, $Value, $PrmLst, $PosBeg, $PosEnd) {
        ...
        for ($c = 0; $c < $col_nbr; $c++)
            $col_lst[$c] = intval($col_lst[$c]); // Conversion into numerical
...
// Add columns by shifting
    ...
    function Ext_PrepareInfo($Ext = false) {
        ...
        if (isset($GLOBAL['_OPENTBS_AutoExt'][$Ext])) {
            ...
            if ($set_option) {
                $TBS->SetOption('parallel_conf', 'tbs:table', array(
                    'parent' => 'table:table',
    ...
    function XML_DeleteElements(&$Txt, $TagLst, $OnlyInner = false) {
        ...
        foreach ($TagLst as $tag) {
            ...
            while (($p1 = $this->XML_FoundTagStart($Txt, $t_open, $p1)) !== false) {
                ...
                $pe1 = strpos($Txt, '>', $p1);
                if ($pe1 === false)
                    return false; // error in the XML formating
                $p2 = false;
...
// delete the tag
    ...
    function Misc_ColNum($ColRef, $IsODF) {
        ...
        if ($IsODF) {
            $p = strpos($ColRef, '.');
            if ($p !== false)
                $ColRef = substr($ColRef, $p); // delete the table name wich is in prefix
    ...
    function OpenXML_Rels_GetObj($DocPath, $TargetPrefix) {
        ...
        if (!isset($o->DirLst[$TargetPrefix])) {
            ...
            while (($p = strpos($Txt, $zTarget, $p + 1)) !== false) {
                ...
                $p2 = strpos($Txt, '"', $p1);
                if ($p2 === false)
                    return $this->RaiseError("(OpenXML) end of attribute Target not found in position " . $p1 . " of subfile " . $o->FicPath);
                $Target = $TargetPrefix . substr($Txt, $p1, $p2 - $p1);
                // Get the Id
                $p1 = strrpos(substr($Txt, 0, $p), '<');
                if ($p1 === false)
                    return $this->RaiseError("(OpenXML) begining of tag not found in position " . $p . " of subfile " . $o->FicPath);
                $p1 = strpos($Txt, $zId, $p1);
                if ($p1 !== false) {
                    $p1 = $p1 + strlen($zId);
                    $p2 = strpos($Txt, '"', $p1);
                    if ($p2 === false)
                        return $this->RaiseError("(OpenXML) end of attribute Id not found in position " . $p1 . " of subfile " . $o->FicPath);
    ...
    function OpenXML_ChartChangeSeries($ChartRef, $SeriesNameOrNum, $NewValues, $NewLegend = false) {
        ...
        if (!isset($this->OpenXmlCharts[$ref]))
            $ref = 'chart' . $ref; // try with $ChartRef as number
        if (!isset($this->OpenXmlCharts[$ref])) {
            ...
            foreach ($charts as $c)
                if ($c['title'] === $ChartRef)
                    $ref = $c['name']; // try with $ChartRef as title
                    if (!isset($this->OpenXmlCharts[$ref]))
                return $this->RaiseError("(ChartChangeSeries) : unable to found the chart corresponding to '" . $ChartRef . "'.");
        }
        ...
        if ($ser === false)
            return $this->RaiseError("(ChartChangeSeries) '$ChartRef' : unable to found series '" . $SeriesNameOrNum . "' in the chart '" . $ref . "'.");
    ...
    function MsExcel_SheetDeleteAndDisplay() {
        ...
        if ($WkbRelsTxt !== false)
            $this->TbsStorePut($this->MsExcel_Sheets_WkbRelsIdx, $WkbRelsTxt);
